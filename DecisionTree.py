...
class Leaf:
    ...
    def __init__(self, rows, id, depth):
        ...
        self.predictedClassLabel = maxCount_label(rows)
        self.id = id
        self.depth = depth
...
def print_tree(node, spacing=""):
    ...
    if isinstance(node, Leaf):
        ...
        print( spacing+ "Predicted Class label ", node.predictedClassLabel  , "     Depth : ", str(node.depth) , "     Id : ", node.id)
        return

    # Print the question at this node
    print (spacing + str(node.question) , "     Depth : ", str(node.depth) , "     Id : ", node.id)
...
def computeAccuracy(rows, node):
    ...
    numAccurate = 0
    for row in rows:
        predictLabel = classify(row,node)
        if predictLabel == row[-1]:
            numAccurate += 1
